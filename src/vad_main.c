...
int add_cut_write_file(struct periods *per, struct cut_info *cut);

int main(int argc, char *argv[]) {
    if (argc != 3) {
        printf("command: vad inFilename outFilename output_dir\n");
        return -2;
    }
...
//    const char output_dir[] = "output_pcm"; // 保存的目录
    ...
    const char *output_filename_prefix = argv[2];
